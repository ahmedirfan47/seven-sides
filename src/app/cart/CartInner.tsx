...
export default function CartInner() {
  ...
  const heatLabel = (h: string) =>
    HEAT_LEVELS.find(l => l.id === h)?.label ?? h;
  ...
  return (
    <div className="container-px mx-auto max-w-6xl py-8 sm:py-12">
      <h1 className="mb-6 font-display text-3xl text-white sm:text-4xl">
        YOUR CART{' '}
        <span className="font-sans text-base font-normal text-dark-100">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
        ...
        <div className="space-y-3">
          {items.map(item => (
            <div
              key={`${item.productId}-${item.heatLevel}`}
            ...
            >
              ...
              <div className="flex flex-1 min-w-0 flex-col gap-1.5">
                <p className="font-semibold text-white text-sm leading-snug line-clamp-1 sm:text-base">
                  {item.name}
                </p>
                ...
                <div className="flex items-center justify-between gap-2 mt-1">
                  <div className="flex items-center overflow-hidden rounded-xl border border-dark-300">
                    <button
                      ...
                      onClick={() =>
                        item.quantity > 1
                          ? updateQuantity(item.productId, item.heatLevel as HeatLevel, item.quantity - 1)
                          : removeItem(item.productId, item.heatLevel as HeatLevel)
                      }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      onClick={() => removeItem(item.productId, item.heatLevel as HeatLevel)}
        ...
        <div className="rounded-2xl border border-dark-300 bg-dark-500 p-5 sm:p-6 lg:sticky lg:top-24">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between text-dark-100">
              <span>Delivery</span>
              <span className="text-dark-200 text-xs">Calculated at checkout</span>
            </div>
          </div>
          ...
          <Link
            href="/menu"
            className="mt-3 flex items-center justify-center gap-1 text-sm font-medium text-dark-100 hover:text-white transition-colors py-2"
          >
            ← Continue Ordering
          </Link>
        </div>
